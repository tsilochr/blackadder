...
/*ATTENTION: I changed that to non-recursive mode- I check on this scope only*/
void Scope::findSubscribers(RemoteHostSet & subscribers) {
...
/*ATTENTION: I changed it to non-recursive mode - now I check for information items on this scope only*/
void Scope::findAllInformationItems(InformationItemSet & _informationitems) {
...
String Scope::printID(unsigned int index) {
    ...
    char *buf;
    if (index < ids.size()) {
        //HashTable<String, Pair<std::set<RemoteHost *>, std::set<RemoteHost *> > *>::iterator it;
