...
/*update the publishers' set with _publisher
...
void InformationItem::findSubscribers(RemoteHostSet &subscribers) {
    //click_chatter("checking for subscribers in information item %s (I use the first ID)", (*ids.begin()).first.quoted_hex().c_str());
    ...
    /*I will not search the father scopes...do it manually by calling fatherScope->findSubscribers()*/
    /*Careful: in the non-recursive RV mode I have to check only the father scopes*/
}

void InformationItem::findPublishers(RemoteHostSet &publishers) {
